...
def getVulFuncAddress(vul, name):
    with open(vul, "rb") as fp:
        ...
        if code.get_symbol_by_name(name):
            global_data["address_vulFunc"] = hex(
                code.get_symbol_by_name(name)[0].entry.st_value)
        if code.get_symbol_by_name("patch_handler"):
            global_data["address_patchHandler"] = hex(
                code.get_symbol_by_name("patch_handler")[0].entry.st_value)
...
#         for find in tmp:
#             c_generate_prefix = c_generate_prefix + \
#                 find[0] + "* " + find[1] + " = NULL;"
...
def setup_args():
    parser = argparse.ArgumentParser(
        prog="gen_patch", epilog="e.g. python3 gen_patch.py -e tmp -f func -s code.c -o patch.json")
    # parser.add_argument("-a", "--asm", metavar="asm", help="compile asm.")
    parser.add_argument("-e", "--elf", metavar="elf",
                        help="set vulnerable elf.")
    parser.add_argument("-f", "--func", metavar="func",
                        help="set vulnerable func.")
